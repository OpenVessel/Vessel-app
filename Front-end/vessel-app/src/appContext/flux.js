	...
	return {
		...
		actions: {
			...
			csrf_token_call: async () => {
				...
				// /api/csrf_token_call
				// This will fail if the redis-server is down
			...
			login: async (username, password) => {
				...
				try{
					...
					setStore({return_msg: data.message, token: data.access_token, username: data.username, email: data.email }); 
					//setStore login view refresh its hooked to Context
			...
			registration: async (token_id, csrf_token, firstname, lastname, username, email, password, confirmpassword) => {
				...
				try{
					...
					sessionStorage.setItem("username", data.username);
					sessionStorage.setItem("nextpageoffer", data.nextpageoffer);
					setStore({return_msg: data.message, 
						firstname:data.firstname, username:data.username,
						nextpageoffer:data.nextpageoffer});
					//setStore login view refresh its hooked to COntext
					return true;
